...
import { Compass, Lock, Loader2, CheckCircle2, ArrowLeft } from 'lucide-react';
...
function ResetPasswordForm() {
  ...
  if (success) {
    return (
      <div className="bg-white rounded-3xl p-8 sm:p-10 max-w-md w-full border border-slate-200 shadow-soft space-y-6 text-center">
        <div className="w-14 h-14 rounded-3xl bg-emerald-100 text-emerald-600 flex items-center justify-center mx-auto">
          <CheckCircle2 className="w-8 h-8" />
        </div>
        <div className="space-y-1">
          <h1 className="text-2xl font-black text-slate-900">Password Reset Complete!</h1>
        ...
        <Link
          href="/login"
          className="w-full py-3 bg-coral hover:bg-coral-dark text-white rounded-xl text-sm font-bold shadow-md shadow-coral/20 flex items-center justify-center gap-2 transition-all"
        >
          Proceed to Sign In
        </Link>
      </div>
    ...
    <div className="bg-white rounded-3xl p-8 sm:p-10 max-w-md w-full border border-slate-200 shadow-soft space-y-6">
      ...
      {!token && (
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs font-semibold text-amber-800">
          Warning: No reset token detected in URL. Please use the link provided in your email or forgot password page.
        </div>
      )}
